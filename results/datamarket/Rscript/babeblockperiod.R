...
input_data <-
  read.csv(file = '/home/renault/Documents/code/results/datamarket/Rscript/csvdata/babeblockperiod.csv')
...
ggplot(data = input_data, aes(x = Period, y = TransactionRate)) + geom_line(aes(colour =
                                                                                Type)) + scale_x_continuous(breaks=c(60,90,120))+ ylim(50, 300) + geom_point() + geom_text_repel(aes(label = round(TransactionRate, 1))) +
  annotate(
    "text",
    size = 3,
    x = 100,
  ...
  ) + ggtitle("Blind Assignment For Blockchain Extension \n Consensus Block Period Influence")


#Save Figure
dev.copy(png, filename = '/home/renault/Documents/code/results/datamarket/Rscript/plots/babeblockperiod.png')
