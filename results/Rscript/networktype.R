...
input_data <- read.csv(file ='/home/renault/Documents/code/results/Rscript/csvdata/networktype.csv')
...
ggplot(data = input_data, aes(x=Node, y=TransactionRate)) + geom_line(aes(colour=Type))+ylim(200, 4000)+geom_point()+geom_text_repel(aes(label = round(TransactionRate, 1)))+
  ...
  ) + ggtitle("Peer Network Type Influence")


#Save Figure
dev.copy(png,filename='/home/renault/Documents/code/results/Rscript/plots/networktype.png');
dev.off ();
