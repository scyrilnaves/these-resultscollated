...
input_data <- read.csv(file ='/home/renault/Documents/code/results/Rscript/csvdata/faultscalabilityinfluence.csv')
...
#head(input_data)
ggplot(data = input_data, aes(x=FaultNodes, y=ScalabilityGradient)) + geom_line(aes(colour=Type))+ylim(0, 0.5)+geom_point()+geom_text_repel(aes(label = ScalabilityGradient)) +
  annotate(
    "text",
    size = 3,
    x = 1.0,
  ...
  coord_cartesian(ylim = c(0, 0.5), clip = "off") + theme(
  ...
  ) + ggtitle("Fault Scalability Gradient") 

#Save Figure
dev.copy(svg,filename='/home/renault/Documents/code/results/Rscript/plots/faultscalabilitygradient.svg');
dev.off ();
