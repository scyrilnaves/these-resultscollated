...
input_data <- read.csv(file ='/home/renault/Documents/code/results/Rscript/csvdata/classification/latencyclassification.csv')
...
ggplot(data = input_data, aes(x=NodeId, y=Epoch)) + geom_point(aes(col=Type,size=Value))+ylim(0, 12)+scale_size(guide=FALSE)+
  ...
  ) + ggtitle("Latency Node Classification")+ scale_y_continuous(limits = c(0, 10), breaks = seq(0, 10, by = 1))



#Save Figure
dev.copy(png,filename='/home/renault/Documents/code/results/Rscript/plots/latencyclassification.png');
dev.off ();
